...
namespace eval Cawt {
    ...
    proc ImgToClipboard { phImg } {
        ...
        if { [package vsatisfies $imgVersion "2.0-"] } {
            set data [string range [$phImg data -format bmp] $sBmpHeaderSize end]
        } else {
            set data [string range [binary decode base64 [$phImg data -format bmp]] $sBmpHeaderSize end]
        }
        twapi::open_clipboard
