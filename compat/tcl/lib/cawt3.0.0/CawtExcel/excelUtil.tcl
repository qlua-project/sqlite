...
namespace eval Excel {
    ...
    proc ClipboardToMatrix { args } {
        # Return the matrix values contained in the clipboard.
        #
        # args - Options described below.
        #
        # -separator <string> - The separation character of the clipboard `CSV` data.
        #                       Default separator is `;`.
        # -timeout <float>    - Timeout in seconds to wait for clipboard data.
        ...
        #
        # Returns the matrix values contained in the clipboard. If no `CSV` data could be
        # retrieved within the timout period, an empty list is returned.
        #
        ...
        if { [llength $args] == 1 } {
            # Old mode with optional string parameter sepChar
            dict set opts -separator [lindex $args 0]
        } else {
            foreach { key value } $args {
                if { [dict exists $opts $key] } {
                    if { $value eq "" } {
                        error "ClipboardToMatrix: No value specified for key \"$key\"."
                    }
                    dict set opts $key $value
                } else {
                    error "ClipboardToMatrix: Unknown option \"$key\" specified."
                }
            }
        }
        ...
        set wait 50
        set accum 0
        set timeoutMs [expr { int ([dict get $opts "-timeout"] * 1000.0) }]
        ...
        Excel SetCsvSeparatorChar [dict get $opts "-separator"]
