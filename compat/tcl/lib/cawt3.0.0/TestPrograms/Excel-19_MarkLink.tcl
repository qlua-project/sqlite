...
set refWorksheetId [Excel AddWorksheet $workbookId "Excel Ref"]
set worksheetId    [Excel AddWorksheet $workbookId "Excel Source"]

# Generate a text file for testing the hyperlink capabilities.
set relFileName "Excel-19_MarkLink File.txt"
set absFileName [file join [pwd] "testOut" $relFileName]
set fp [open $absFileName "w"]
puts $fp "This is the text file linked from Excel."
close $fp
...
Excel SetHyperlink $worksheetId 3 1 [format "file://%s" $absFileName] "Hyperlink to file"
Excel SetHyperlink $worksheetId 4 1 [format "http://%s" "www.cawt.tcl3d.org"] "Hyperlink to URL"

# Create a hyperlink to a file using an absolute and relative path.
Excel SetHyperlinkToFile $worksheetId 5 1 $absFileName "Absolute hyperlink to file"
Excel SetHyperlinkToFile $worksheetId 6 1 $relFileName "Relative hyperlink to file"
...
set commentId [Excel SetRangeComment $rangeId "Comment text." [file join [pwd] "testIn/wish.gif"] false]
Excel SetCommentSize $commentId 3c 5c
Cawt CheckString "Comment text." [Excel GetCellComment $worksheetId 5 3] "Cell comment"
Cawt CheckString "" [Excel GetCellComment $worksheetId 6 3] "Cell comment"
