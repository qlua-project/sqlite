...
foreach fr $frameList mode $modeList {
    ...
    set t1 [clock clicks -milliseconds]
    Excel WorksheetToTablelist $worksheetId $fr.frOut1.tl -header $useHeader -rownumber $useRowNum
    set t2 [clock clicks -milliseconds]
    ...
    set t1 [clock clicks -milliseconds]
    Excel WorksheetToTablelist $worksheetId $fr.frOut2.tl -header $useHeader -rownumber $useRowNum
    set t2 [clock clicks -milliseconds]
    puts "WorksheetToTablelist: [expr $t2 - $t1] ms (Mode $mode using header: $useHeader)."
    if { $mode ne "Numbered" } {
