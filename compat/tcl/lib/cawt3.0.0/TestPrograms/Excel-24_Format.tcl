...
# If "--English" is specified, a dot is used as floating point separator and
# a comma as thousands separator.
# If "--German"  is specified, a comma is used as floating point separator and
# a dot as thousands separator.
#
# Note, that if using one of these 2 options, the standard settings regarding
# separators are changed globally for the installed Excel application.
#
...
incr row
# Insert number as currency. 
# Use predefined values for Euro, Dollar, Pound, Yen and "Deutsche Mark" (RIP).
set numberFormat [Excel GetCurrencyFormat $appId "Euro" "0" "00" $floatSep]
InsertValue $worksheetId $row "12345.6" $numberFormat  ; incr row

set numberFormat [Excel GetCurrencyFormat $appId "Dollar" "0" "00" $floatSep]
InsertValue $worksheetId $row "12345.6" $numberFormat  ; incr row

set numberFormat [Excel GetCurrencyFormat $appId "Pound" "0" "00" $floatSep]
InsertValue $worksheetId $row "12345.6" $numberFormat  ; incr row

set numberFormat [Excel GetCurrencyFormat $appId "Yen" "0" "00" $floatSep]
InsertValue $worksheetId $row "12345.6" $numberFormat  ; incr row

set numberFormat [Excel GetCurrencyFormat $appId "DM" "0" "000" $floatSep]
InsertValue $worksheetId $row "12345.6" $numberFormat  ; incr row

# Insert number as currency. Use any Unicode character. Example: German Penny Sign.
set numberFormat [Excel GetCurrencyFormat $appId "\u20B0" "0" "00" $floatSep]
InsertValue $worksheetId $row "12345.6" $numberFormat  ; incr row
