...
set inFile [file join [pwd] "testIn" "ReportTemplate.docx"]
...
set headerText [Word GetHeaderText $docId wdHeaderFooterPrimary]
puts "Header text: $headerText"
set footerText [Word GetFooterText $docId 1]
puts "Footer text: $footerText"
...
for { set s 1 } { $s <= $numTestSuites } { incr s } {
    ...
    for { set t 1 } { $t <= $numTestCases } { incr t } {
        ...
        set tableRange [Word CreateRangeAfter $titleRange]
        set tableId [Word AddTable $tableRange 2 2]
        set tableName [format "Table-%d.%d" [expr $s + 1] $t]
        Word SetTableName $tableId $tableName
        Word SetTableBorderLineStyle $tableId
        Word SetHeaderRow $tableId [list "Requirement" "Test Case"]
        Word SetCellValue $tableId 2 1 "Requirement $t"
        Word SetCellValue $tableId 2 2 "Tested by $title"
...
Word SetTableName $tableId "SummaryTable"
Word SetTableBorderLineStyle $tableId
Word SetHeaderRow $tableId [list "Requirement" "Test Case"]
set req 1
set row 2
