...
proc InsertWithSetRowValues { appId workbookId rowList testName hideApp } {
    ...
    if { $hideApp } {
        ...
        Cawt CheckBoolean false [Excel Visible $appId] "Excel Visible"
    }
    set startRow 1
    set startCol 1
    ...
    if { $hideApp } {
        ...
        Cawt CheckBoolean true [Excel Visible $appId] "Excel Visible"
    }
    set t2 [clock clicks -milliseconds]
    puts "[expr $t2 - $t1] ms to set row values in $testName mode."
}

proc CheckWorksheets { workbookId worksheetPrefix } {
    global numRows numCols numWorksheets

    set startRow 1
    set startCol 1
    for { set ws 0 } { $ws < $numWorksheets } { incr ws } {
        set wsName "$worksheetPrefix-$ws"
...
CheckWorksheets $workbookId "RowVisible"
