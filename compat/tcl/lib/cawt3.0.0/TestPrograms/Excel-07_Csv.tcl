...
set outFileNoNewlineCsv [file join $outPath Excel-07_Csv_NoNewlineCsv.csv]

# Names of Excel file being generated.
set outFileExcel2 [file join $outPath Excel-07_Csv_Excel]
...
# Read and write the generated CSV file with the ExcelCsv procedures. 
...
# Read the generated CSV file with the ExcelCsv procedure,
# copy the data into a worksheet and write it as an Excel file.
set appId [Excel Open]
set workbookId  [Excel AddWorkbook $appId]
set worksheetId [Excel AddWorksheet $workbookId "CsvSep"]
puts "Reading CSV file $outFileExcel"
set csvMatrix [Excel ReadCsvFile $outFileExcel]
Excel SetMatrixValues $worksheetId $csvMatrix
append outFileExcel2 [Excel GetExtString $appId]
puts "Saving CSV data as Excel file $outFileExcel2"
Excel SaveAs $workbookId $outFileExcel2 "xlWorkbookDefault"
Excel Close $workbookId
Excel Quit $appId false
...
# Replace cell internal newlines with spaces.
set noNewlineMatrix [list]
foreach rowList $excelMatrix {
    set noNewlineRowList [list]
    foreach col $rowList {
        lappend noNewlineRowList [string map { "\n" " " } $col]
    }
    lappend noNewlineMatrix $noNewlineRowList
}
puts "Writing CSV file $outFileNoNewlineCsv"
Excel WriteCsvFile $noNewlineMatrix $outFileNoNewlineCsv
