#
# GUI integration tests for windetect, specific for Tk versions that invoke
...
test pointer_motion-1.1 {into and out of a window, container = parent} -constraints includeWmNotInvolved -setup {
	...
	no_leak_drawing_events tk
		...
		windetect "{.f1.f2 <Enter> .f1.f2} {.f1 <Enter> .f1}" \
		tkwintrack {}]
...
test pointer_motion-1.4 {into a grandchild window and back again, include virtual crossing} -constraints includeWmNotInvolved -setup {
	...
	pointer_move .f1.f2 c
	waitForReporting 3; # including one virtual crossing <Enter> event
	record state(1,outline) outline .f1.f2
	record state(1,pnw) pnw
	pointer_move . c
	waitForReporting 3; # including one virtual crossing <Leave> event
	...
	unset -nocomplain state reportingLog
		...
		windetect "{.f1.f2 <Enter> .f1.f2} {. <Enter> .}" \
		tkwintrack {}]
...
test grabbing-2.3 {Enter and leave a contained window through consecutive [grab release]+[pack forget] (container = parent)} -constraints includeWmNotInvolved -setup {
	...
	unset -nocomplain state reportingLog
		...
		windetect-noDelay "{.f1 <Enter> .f1} {. <Enter> .}" \
		windetect-defaultDelay "{. <Enter> .}" \
		tkwintrack {}]
...
test pw_mapping-2.1 {Enter two packed widgets consecutively through a single [pack]} -constraints includeWmNotInvolved -setup {
	...
	pack forget .f1.f2
	pack forget .f1
	waitForReporting [expr {[tk windowingsystem] eq "x11"?4:3}]
	pointer_move . c
	if {$Generic(pkgName) eq "tkwintrack"} {
		interp eval tkwintrack pnw.recall
		update; # enforce redraw of pnw
...
test pw_mapping-2.2 {Leave two packed widgets consecutively through a single [pack forget]} -constraints includeWmNotInvolved -setup {
	...
	update
	pointer_move .f1.f2 c
	waitForReporting 3; # including one virtual <Enter> event
	set reportingLog {}
} -body {
	pack forget .f1
	waitForReporting 3; # including one virtual <Leave> event
	record state(outline) outline .
	record state(pnw) pnw
	if {$Generic(pkgName) eq "tkwintrack"} {
		concat $state(outline) $state(pnw)
...
#
# The only window managers that seem to comply are x11.Fluxbox, x11.KWin and
# win32.win32_WM_NR.
#
...
test pw_destroy-1.1 {a packed window} -constraints {includeWmNotInvolved bug_Tk_22349fc78a_withDetectionDetails} -setup {
	pack .f1
	update
	pointer_move .f1 c
	waitForReporting 2
	...
	destroy .f1
	waitForReporting 2 1; # the <Destroy> event was serviced before the destroy command returned
	record state(outline) outline .
	record state(pnw) pnw
	if {$Generic(pkgName) eq "tkwintrack"} {
		concat $state(outline) $state(pnw)
...
test pw_destroy-1.2 {Destruction of pointer window through destruction of its container window, single [destroy]} -constraints {includeWmNotInvolved bug_Tk_22349fc78a_withDetectionDetails} -setup {
	pack .f1.f2
	pack .f1
	update
	pointer_move .f1.f2 c
...
test pw_destroy-2.1 {Consecutive destruction of contained windows, two separate calls to [destroy]} -constraints {includeWmNotInvolved bug_Tk_22349fc78a_withDetectionDetails} -setup {
	pack .f1.f2
	pack .f1
	update
	pointer_move .f1.f2 c
	...
	destroy .f1.f2
	destroy .f1
	waitForReporting 3 2; # both detection events happened before the respective destroy commands returned
	record state(outline) outline .
	record state(pnw) pnw
	if {$Generic(pkgName) eq "tkwintrack"} {
		concat $state(outline) $state(pnw)
...
test pw_destroy-2.2 {Consecutive destruction of pointer window and unmapping its container window, [destroy]+[pack forget]} -constraints {includeWmNotInvolved bug_Tk_22349fc78a} -setup {
	pack .f1.f2
	pack .f1
	update
	pointer_move .f1.f2 c
	...
	destroy .f1.f2
	pack forget .f1
	waitForReporting 4 1; # the <Destroy> detection event happened before the destroy command returned
	record state(outline) outline .
	record state(pnw) pnw
	if {$Generic(pkgName) eq "tkwintrack"} {
		concat $state(outline) $state(pnw)
...
test pw_destroy-2.3 {Consecutive destruction of pointer window and mapping another one, [destroy]+[pack]} -constraints {includeWmNotInvolved bug_Tk_22349fc78a} -setup {
	pack .f1
	update
	pointer_move .f1 c
	waitForReporting 2
	...
	destroy .f1
	pack .g -pady 20
	waitForReporting 4 1; # the <Destroy> detection event was delivered/handled before the destroy command returned
	record state(outline) outline .g
	record state(pnw) pnw
	if {$Generic(pkgName) eq "tkwintrack"} {
		concat $state(outline) $state(pnw)
	...
	no_leak_drawing_events tk
		...
		windetect-noDelay "{. <Enter> .} {.g <Enter> .g}" \
		windetect-defaultDelay "{.g <Enter> .g}" \
		tkwintrack {}]

test pw_destroy-2.4 {Consecutive destruction of pointer window and mapping another one with reverse calling order, [destroy]+[place]} -constraints {includeWmNotInvolved bug_Tk_22349fc78a} -setup {
	pack .f1
	update
	pointer_move .f1 c
	waitForReporting 2
	...
	#
	place .g -in . -x 85 -y 25; # - schedule screen redraw for .g at idle time [B1]
	destroy .f1;          # - schedule screen redraw at idle time: remove .f1 and expose . [B2]
						  # - queue an <Enter> event on . [A1]
						  # - generate a <Destroy> event, to be serviced immediately
						  # - service <Destroy> event, calling [handleDetectionEvent], scheduling [report] at idle time [B3]
						  # - [destroy .f1] returns
	waitForReporting 3;	  # - Service queued events:
						  #   [A1] <Enter> event on ., invoking [handleDetectionEvent], scheduling a [report] at idle time [B4]
						  # - The event queue is emptied, service all idle tasks:
						  #   [B1] screen redraw: .g is drawn, queueing an <Enter> event on .g [A2]
						  #   [B2] screen redraw .f1 is removed from the screen
						  #   [B3] invoke [report] (for <Destroy>), which waits[*] for the next <Enter> event
						  #   While waiting:
						  #   [B4] invoke [report] for <Enter> ., releasing the waiting[*], and invoke the reporting callback
						  # - [report] for <Destroy> returns without having called a reporting callback
						  #   [A2] <Enter> event on .g, scheduling a [report] at idle time [B5]
						  #   [B5] invoke [report] for <Enter> .g and invoke the reporting callback
						  #
	record state(outline) outline .g
	record state(pnw) pnw
	if {$Generic(pkgName) eq "tkwintrack"} {
...
test pw_destroy-2.5 {Destruction of pointer window through destruction of its container window, and mapping another one, single [destroy]+[pack]} -constraints {includeWmNotInvolved bug_Tk_22349fc78a} -setup {
	pack .f1.f2
	pack .f1
	update
	pointer_move .f1.f2 c
	...
	destroy .f1
	pack .g
	waitForReporting 5 2; # both <Destroy> detection events happened before the destroy command returned
	record state(outline) outline .g
	record state(pnw) pnw
	if {$Generic(pkgName) eq "tkwintrack"} {
		concat $state(outline) $state(pnw)
...
test pw_destroy-3.1 {a grabbed pointer window} -constraints {includeWmNotInvolved bug_Tk_22349fc78a_withDetectionDetails} -setup {
	pack .f1
	update
	grab .f1
	waitForReporting
	...
	destroy .f1
	waitForReporting 2 1; # the detection event happened before the destroy command returned
	record state(outline) outline .
	record state(pnw) pnw
	if {$Generic(pkgName) eq "tkwintrack"} {
		concat $state(outline) $state(pnw)
