...
package ifneeded perlin 1.0.2 [list apply { dir {
	...
	switch -- $majorVersion {
		8 {set vtag "85"}
		9 {set vtag "90"}
		default { error "perlin: Unsupported Tcl version" }
	}

	set tail_libFile perlin${vtag}[info sharedlibextension]
	 # try to guess the tcl-interpreter architecture (32/64 bit)
	set arch $::tcl_platform(pointerSize)
	switch -- $arch {
		4 { set arch x32  }
		8 { set arch x64 }
		default { error "perlin: Unsupported architecture: Unexpected pointer-size $arch!!! "}
	}

	set dir_libFile [file join $thisDir ${os}-${arch}]
	if { ! [file isdirectory $dir_libFile ] } {
		error "perlin: Unsupported platform ${os}-${arch}"
	}

	set full_libFile [file join $dir_libFile $tail_libFile]
	load $full_libFile

	package provide perlin 1.0.2
