...
# ? e mettere un pattern granuloso per la linea ??
set borderStyle [BL::color black 0.3]
		dict set my(tileData) "border" [list "stroke" $hexShape "-style $borderStyle -width 1.5"]
# forse bordi di colore diversi (quello sul bianco molto piu' leggero)


# va be .. non capisco l'ordine
		set style3 [BL::color gray40]
		set style2 [BL::color gray65]
		set style1 [BL::color gray90]  ;# top

		set points [HexCellCorners {0 0} $radius]
#fai loop e fai proc per prendere punti ..
		set face [BL::Path new]
		set facePoints [list {0 0} [lindex $points 3] [lindex $points 4] [lindex $points 5]]
		$face add [BL::polygon {*}$facePoints]
		dict lappend my(tileData) "fg" "fill" $face "-style $style1"

		set face [BL::Path new]
		set facePoints [list {0 0} [lindex $points 5] [lindex $points 0] [lindex $points 1]]
		$face add [BL::polygon {*}$facePoints]
		dict lappend my(tileData) "fg" "fill" $face "-style $style2"

		set face [BL::Path new]
		set facePoints [list {0 0} [lindex $points 1] [lindex $points 2] [lindex $points 3]]
		$face add [BL::polygon {*}$facePoints]
		dict lappend my(tileData) "fg" "fill" $face "-style $style3"
	}

	destructor {
		if { [info exists my(tileData)] } {
			foreach key {"border" "fg"} {
	...
	method draw {sfc x y angle scale {hasBorder false}} {
		...
		foreach {op path opt} [dict get $my(tileData) "fg"] {
			$sfc $op $path {*}$opt
		}
		if {$hasBorder} {
	...
	method fillAll {} {
		...
		for {set y 0; set row 0} {$y-$dy/2 <$HEIGHT} {set y [expr {$y+$dy}]; incr row} {
			...
			for {set x $x; set col 0} {$col < $cols} {set x [expr {$x+$dx}]; incr col} {
				set angle [expr {(rand()<0.5) ? 0 : 180}]
				my _SetRotation [list $col $row] $angle
				$my(hexTile) draw $my(sfc) $x $y $angle 1.0 true
			}
		}	
	}
	...
	method _co_animateRunningCells {} {
		while { [dict size $my(RunningCells)] > 0 } {
			...
			foreach cell [dict keys $my(SurroundingCells)] {
				...
				$my(hexTile) draw $my(sfc) {*}$xy $rotation 1.0 true
			}
